...
const CustomerAddress = require('../../models/customer/customerAddressModel');
...
const addLocation = async (req, res) => {
  const result = await customerAddLocationSrvice(req, CustomerAddress);
  if (result.data) {
    res.status(200).json(result);
  } else {
    res.status(400).json(result);
...
const getLocations = async (req, res) => {
  ...
  try {
    const locations = await CustomerAddress.find({ userId: id });
...
const deleteLocation = async (req, res) => {
  const { id } = req.params;
  try {
    const result = await CustomerAddress.deleteOne({ _id: id });
    res.status(200).json({ status: 'Success', data: result });
  } catch (error) {
    res.status(400).json({ status: 'fail', data: error.toStrign() });
  }
